...
- `ServiceItem`：禁用或恢复 Windows 服务；服务名以 `*` 结尾时按前缀匹配（如 `CDPUserSvc*`）。
- `ScheduledTaskItem`：禁用或恢复一个或多个计划任务（`bin\Data\ScheduledTaskItems.tab`）。
- `DriverItem`：按驱动路径或 Windows 服务检测第三方杀软/EDR；优化时尝试删除特征文件/目录，失败时引导用户手动卸载。
- `MicrosoftStoreItem`：卸载指定 Microsoft Store 包。

新增简单优化项时，优先考虑修改 `bin\Data` 下对应的 `.tab` 文件，而不是新增代码类。

取舍与检测成本（Win10 和 Win11 都要考虑）：

- 只收录对性能或体验有实际收益的项。商店卸载项只保留会常驻后台或开机自启的应用；平时不运行的按需服务、很少触发的计划任务不收录。
...
- 商店项的检测和卸载都只针对当前用户，两边范围一致。不用 `-AllUsers`：它还会列出卸载后 Windows 留给新账户的暂存副本（系统账户 S-1-5-18、状态 Staged），卸载后重启仍会显示需要卸载；其他账户和预配副本也不替用户处理。已知限制：以另一个管理员账户提权运行时，“当前用户”是那个管理员。Debug MCP `store_package_probe` 可查看单个包的检测结果和各账户状态。
