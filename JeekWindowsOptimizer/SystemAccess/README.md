...
`SystemAccess` 目录封装了优化项需要访问的 Windows 系统能力。这里的类型不负责 UI 展示，也不直接管理优化项生命周期，而是为 `OptimizationItem` 和 `MainViewModel` 提供可复用的底层操作。
...
- 新增系统能力时，优先在 `SystemAccess` 中封装底层调用，再由优化项调用。
